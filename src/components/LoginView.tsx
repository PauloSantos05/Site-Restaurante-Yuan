...
export default function LoginView({ onNavigate, onLoginSuccess }: LoginViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setGeneralError(data.erro || "Falha nas credenciais imperiais. Verifique seu login.");
        setIsLoading(false);
        return;
      }

      onLoginSuccess({
        fullName: data.nome || "Dignitário Imperial",
  ...
  return (
    <main className="page relative min-h-screen flex items-center justify-center">
      ...
      >
        {/* Return Button inside card header */}
        <button
        ...
        {/* Subtitle */}
        <p className="subtitle text-sm text-[#9A9180] font-light mb-8">Bem-vindo de volta</p>
        ...
        <form className="form w-full flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
          ...
          <button
            ...
            className="forgot-link text-left text-xs text-[#7A9E82] hover:underline mb-2"
            id="linkEsqueciSenha"
          >
            Esqueci minha senha
          </button>
          ...
          <button type="submit" className="btn-entrar" id="btnEntrar" disabled={isLoading}>
            {isLoading ? (
              <>
                ...
                <span className="btn-entrar__text opacity-70">Acessando...</span>
              </>
            ) : (
              <span className="btn-entrar__text">Entrar</span>
            )}
          </button>
        </form>
        ...
        <p className="register-line text-xs font-light text-[#9A9180] mt-6">
          Ainda não tem conta?{" "}
          <button onClick={() => onNavigate(Screen.Register)} className="register-link text-[#7A9E82] font-semibold hover:underline">
            Cadastre-se
          </button>
        </p>
      </motion.div>
